...
>
> **Packaging note:** For WAV processing, copy `MagicLanternTagger.exe` and `ffmpeg.exe` into the music folder together. Keep `README.md`, `ffmpeg-LICENSE`, and `ffmpeg-README.txt` in the distribution package.
...
2. **실행**
   ...
   - 자동으로 모든 음악 파일 처리됨
   - WAV 파일은 원본을 유지하면서 `mp3` 하위 폴더에 MP3 사본도 생성됨
...
2. **Run**
   ...
   - WAV files keep their original file and also create an MP3 copy in the `mp3` subfolder
...
2. **実行**
   ...
   - WAV ファイルは元のファイルを残したまま、`mp3` サブフォルダに MP3 のコピーも作成されます
